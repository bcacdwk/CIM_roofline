...
**本轮处理：** 降为补充；统一输入时序优先由 SACIM-03 的实际 CIM 路径支撑。
...
**本轮处理：** 降为端口冲突与局部边界补充；C 轮补 CMOS-07。
...
**本轮处理：** 从通用输入基线候选降为其他负载下的 28 nm 交叉参考。
...
**本轮处理：** 保留设计参考，不能将 headline GS/s 标成实测。
...
**本轮处理：** 新增共同感测／SRAM 服务核心候选，补 CMOS-04 未直接给周期信息的缺口；已在用户停止下载指令前归档。
...
**本轮处理：** 提升为共同 28 nm 输入／求值桥接的核心来源；纠正仅依据会议前作的仿真印象。
...
**本轮处理：** 补正写端口为 128 bit，避免与输出维度 16 混淆。
...
**本轮处理：** 修正厂家归属为 Sandisk/KIOXIA，取消“完全没有 SLC 模式来源”的笼统缺口。
...
**本轮处理：** C 轮补 RRAM-06 的本地 page SET/RESET 控制证据。
...
**本轮处理：** 保留真实慢产品作为完整周期／层级反例，不能因不符期望量级而删除。
...
**本轮处理：** 纠正 headline 优化值可能被误当实测及“多 bit 等于多级 RRAM cell”的风险。
...
**本轮处理：** 降为近存数字替代情景；已自动补齐真正的 13 页 SI；C 轮另找 MRAM-06 的 bitcell 内数字乘法。
...
**本轮处理：** 接受更新版本，不算缺件；额外 2023 brief 移入 versions，退出有效证据计数。
...
**本轮处理：** C 轮新增 PCM-06，同一 IBM 技术线的细节补充，不算独立工艺交叉验证。
...
**[PCM-04](LITERATURE_CATALOG.md#PCM-04) — 条件保留** · [本地 PDF](literature/07_pcm/PCM-04_2017_Subnanosecond_Nucleation.pdf)

**正文定位：** pp.2–4，Figs.2–4；p.4，Supplementary Materials 声明。

**保留用途：** Sc-Sb-Te 器件原始写入／材料结果可核查快速结晶，与 GST 系芯片构成材料条件对照。

**不能据此声称：** Materials and Methods、RESET 补充图 S6、测量装置及脉冲条件在缺失 SI；亚纳秒不能代替整阵列闭环写入。

**本轮处理：** 主文有用但 SI 是关键缺件，列入 C；不重复请求主文。
...
**本轮处理：** 正文与图像确认材料归属及恢复周期；PDF 字体编码异常但页面正常，不需要重新下载。
...
**本轮处理：** 保留 HfO₂ 电容测量，降低纯 FeRAM 求值桥接权重。
...
**本轮处理：** 归入 FENOR-04 同一证据包，保留原稳定 ID 与版本文件。
...
**本轮处理：** 退出 FeNOR 核心，保留在对照目录并显式命名 FeNAND。
