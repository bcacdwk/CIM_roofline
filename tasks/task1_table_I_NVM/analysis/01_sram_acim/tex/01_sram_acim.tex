...
三组 $\rho$ 为0.0166--0.0831GB/s，$\tau$ 为1.6--7.27GB/s，成对ridge为0.01039--0.01246。后者较窄取决于这三组读写预算的配对变化；将端点独立混合会得到约0.002286--0.05195的外包络，不能冒充同一条件下的实测范围。为单独暴露关键前端不确定性，固定 $T_A=20$ns、$T_D=5$ns、$T_M=5$ns，再扫 $T_F$；另只改变一次真实写并行资源。
