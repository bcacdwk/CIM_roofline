...
- [中文分析 PDF](output/sram_dcim.pdf)：5 页，含两路完整服务、成对范围、R0 资源对照和参数证据表。
...
主情景范围：ρ=0.02490–0.05803 GB/s，τ=1.600–7.273 GB/s，成对 RI*=0.007980–0.01556。参考点为 ΔS=2570 ns、ΔR=5 ns、ρ≈0.04981 GB/s、τ=3.2 GB/s、RI*≈0.01556。B_S=128 Byte，B_R=16 Byte。
...
写事务在本地memory口同沿接收命令/地址/128-bit数据，到下一可计算周期结束；完整memory周期替代已经覆盖的共同写前端。因此主情景不重复加T_W。若确有额外任务握手，参考ΔR=15 ns、τ≈1.067 GB/s、RI*≈0.04669，单列为边界敏感性。R0的32项同钟资源对照也单列，未替换主情景。
