...
def table_results(result):
    ...
    for r in result['scenarios']:
        x=r['inputs']
        lines.append(f"{r['label']} & {x['common_T_D_ns']:g} & {x['complete_compute_round_ns']:g} & {r['delta_S_ns']:g} & {r['delta_R_ns']:g} & {r['rho_Byte_per_s']/1e6:.4g} & {r['tau_Byte_per_s']/1e6:.4g} & {r['ridge']:.4g}"+r'\\')
    lines += [r'\bottomrule\end{tabular}',r'\caption{D6CIM型16项结构的成对情景。所有时间单位为ns，吞吐为十进制MB/s；$\Delta_R=T_M$。时间列保留复算值，非实测有效位数。}\label{02_sram_dcim:tab:dcim-results}\end{table}']
    return '\n'.join(lines)+'\n'
...
def evidence_tex():
    lines=[r'% Generated from data/inputs.json raw_evidence; original and adopted values separated.',
           ...
           r'\caption{参数证据与采用方法。页码均为本地PDF页序；原始值与本文预算分列。}\label{02_sram_dcim:tab:dcim-evidence}\\',
