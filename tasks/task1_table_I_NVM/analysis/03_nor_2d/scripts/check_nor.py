...
def compute():
    ...
    return dict(case_id=I['case_id'],baseline_id=s.D['baseline_id'],mode=I['mode'],
        units=dict(payload='Byte',interval='ns',rate='Byte/s',display_rate='decimal GB/s',ridge='dimensionless'),
        ...
        independent_ridge_envelope=[min(r['rho_Byte_per_s'] for r in main)/max(r['tau_Byte_per_s'] for r in main),
            max(r['rho_Byte_per_s'] for r in main)/min(r['tau_Byte_per_s'] for r in main)],
...
def tex_results(d):
    lines=[r'% Generated by scripts/check_nor.py --emit.',r'\begin{table}[htbp]\centering\small',
        ...
        r'情景 & $\Delta_S$ ($\mu$s) & $\Delta_R$ (ms) & $\rho$ (GB/s) & $\tau$ (GB/s) & $\mathrm{RI}^{*}$\\\midrule']
    for label,row in zip(['短','参考','长'],d['scenarios']):
        lines.append(f"{label} & {row['delta_S_ns']/1000:.3f} & {row['delta_R_ns']/1e6:.6f} & {row['rho_Byte_per_s']/1e9:.4g} & {row['tau_Byte_per_s']/1e9:.4g} & {row['ridge']:.4g}\\\\")
    lines += [r'\bottomrule\end{tabular}',r'\caption{同一 binary 映射的三组完整服务。每行 $B_S=128$ Byte、$B_R=16384$ Byte；显示位数仅供复算。}\label{tab:nor-results}\end{table}']
    return '\n'.join(lines)+'\n'

def tex_contrast(d):
    lines=[r'% Generated by scripts/check_nor.py --emit.',r'\begin{table}[htbp]\centering\small',
        r'\begin{tabular}{@{}lrrrrr@{}}\toprule',
        r'组织 & 分配容量 & 擦除数 & $\Delta_R$ (ms) & $\tau$ (GB/s) & $\mathrm{RI}^{*}$\\\midrule']
    for label,row in [('主布局',d['scenarios'][1]),('独占 sector',d['organization_contrast'])]:
        lines.append(f"{label} & {row['mapping']['allocated_physical_Byte']//1024} KiB & {row['mapping']['sector_erases']} & {row['delta_R_ns']/1e6:.6f} & {row['tau_Byte_per_s']/1e9:.4g} & {row['ridge']:.4g}\\\\")
    lines += [r'\bottomrule\end{tabular}',r'\caption{保持 32 个读分片、4096 SA 和参考读时间，只改变读分片与擦除域的归属。两行 $\rho$ 相同，物理容量不要求相等。}\label{tab:nor-contrast}\end{table}']
    return '\n'.join(lines)+'\n'
