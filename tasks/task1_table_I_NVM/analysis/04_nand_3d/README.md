...
- [中文报告PDF](output/nand_3d.pdf)（7页）；[连贯章节](tex/04_nand_3d.tex)；[独立编译入口](tex/nand_3d.tex)。
...
主情景ρ=0.07507–0.07655 MB/s；此范围固定16 pF、0.4 V、2 nA及25 μs积分，不能表示全部读侧不确定性。预擦除append的τ=3.199–12.30 kB/s、RI*=6.221–23.47；同地址持续整矩阵重写的τ=1.347–4.571 kB/s、RI*=16.75–55.73。参考点ΔS=1.687682 ms；append/持续重写ΔR分别为2.560644/7.040785 s。范围由三个成对参考预算生成，单位kB为10³ Byte。
...
修改输入后执行`/opt/anaconda3/bin/python scripts/check_nand.py --emit`刷新本案例JSON与表格。默认检查只读；通过importlib导入共享计算接口并禁用pycache。当前8组案例检查通过，XeLaTeX及7页渲染目视检查通过，日志无Underfull/Overfull、缺字或未定义引用。检查证明计数、单位和同步，不能替代前端/程序实现能力论证。
