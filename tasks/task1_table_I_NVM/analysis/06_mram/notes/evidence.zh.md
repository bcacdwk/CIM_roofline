...
回查 MRAM-06 PDF p.2/4/9，保留先双P后单AP与写验重写语义；MRAM-03 PDF p.6–7支持完整读写access量级，概率地板及union bound仅留本笔记。脚本不再计算未参与服务的`retry_proxy_q`。特别订正：p.7左栏文字和页面图像均为连续读超过 **1E6** 次无扰动，旧正文及本笔记误写1E16；已同步改为10^6，仍不当写耐久。这项证据订正不改变服务数值。

推荐一次完整尝试的参考点仍为2570/95 ns、ρ=49.8054 MB/s、τ=84.2105 MB/s、ridge=0.591440；一次额外整组重写对照为180 ns、τ=44.4444 MB/s、ridge=1.12062。无周期维护，所列原始占用即给定验收情景的服务间隔。
