...
def generated():
    ...
    s=r['comparison']
    lines.extend([r'\midrule',f"参考，重写一次 & 5/30 & {s['delta_S_ns']:g} & {s['delta_R_ns']:g} & {s['rho_Byte_per_s']/1e6:.3f} & {s['tau_Byte_per_s']/1e6:.3f} & {s['ridge']:.4f}\\\\",
                  r'\bottomrule\end{tabular}',
                  r'\caption{条件配对情景，MB/s 为十进制：$B_S=128$ Byte，$B_R=8$ Byte。前三行为单轮完整写验通过的有限预算；末行为恰好一次整组重写且随后成功的占用对照，未纳入前三行情景范围。}',r'\end{table}'])
