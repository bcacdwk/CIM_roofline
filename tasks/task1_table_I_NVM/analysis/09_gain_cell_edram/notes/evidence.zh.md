...
已再次回查 GC-04 原 PDF pp.9–10 的文字及 Fig.16–18：65 ns 是包括 5 ns 粗写的完整实测测试点；0.4 ms 的 99.7% 是端点样本漂移小于原 ADC 1 LSB，而非新的全阵列精确保持保证。未改变器件、刷新周期或写入分相。

普通情景仅保留短/参考的配对工程预算，推荐参考值不变；长情景单列刷新临界压力。固定长点共享外围与 75 ns 完整编程后，刷新加禁发占用为 `1025 t_m+179280 ns`；可行必须 `t_m<215.33658536585366 ns`，现有 200 ns 仅余 15.3366 ns/组。定向 216 ns 检查得到 400680 ns/400000 ns、α=−0.00170；全部维护后服务间隔/能力/ridge 置空，保留原始服务和 α。脚本另检查 α=0 的同样行为。该不可行点是同一调度的压力检查，不是新的器件参数。

参考原始 ρ=5.403123680878007 MB/s、τ=200 MB/s；按 α=0.3339375 后为 1.8043056141831996/66.7875 MB/s。33.39% 有用时间每周期对应 133.575 µs，显著大于 185 ns 不可中断组；有限资源组织有调度余量，但维护代价很高。共同 α 抵消只保留需求交点比例，不保留吞吐。既有 wide32 仍单独作为资源对照，不扩入普通范围。
