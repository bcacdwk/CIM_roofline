...
## 本轮独立判断与修订决策（2026-09-10）

按指定顺序阅读入口、v0.1 两节及 PDF、参数和证据、计量约定，再阅读旧稿两段。判断：**v0.1 计数自洽，但结构实例与通用模板混在了一起。** ACIM 的 64 来自 8 个输入 bit × 8 个输出组且每组重新求值；数字两拍/批又依赖 16 条输出通道与八个平面同时提供结果。DCIM 的 256 来自 8 输入 bit × 4 行组 × 8 输出组。微秒级耗时是这些组织的结果，不能据其量级判错，也没有依据把它们提升为所有介质的必需次数。

保留四块外围范围、逻辑任务/边界/精度合同、单更新域、原实例全部结构和两个示意的六个原计算点。修订以操作分工为核心：求值组由实际同时产生的输出数确定，ADC 批次由样本与转换资源确定，数字轮数由输出通道与每轮功能确定。保持方案只减少求值次数，转换数和逻辑 payload 不因此减少。直接更新/完整编程/verify/擦除摊销保持原分类逻辑，增加完整权重、物理批数和编码装入拍数的统一写法。

推荐仍以原结构 R0 作为默认代入和跨版本对照，以新的 A/B/C/D 公式作为方法入口。原生限制按共同规则形成有证据的分组与例外，不通过任意选择结构优化介质排名。旧稿只启发四类直观模板的组织；其 ADC 输出字节公式、bank/层数倍率及数值均未恢复。

### 本轮针对性回查

重新打开 CMOS-02 pp.7–9、SACIM-05 pp.2–4、SDCIM-01 pp.1–2 本地 PDF。前者核对完整采样时隙，后二者核对输入分组、采样保持阶段、位权重构和空间复用。没有外部检索或下载。

- CMOS-02 的 100 MS/s 及 ENOB 条件与 v0.1 笔记一致；SAR 内部单样本采样不能直接充当跨八个共享组的长保持资源。
- SACIM-05 p.3 的电容/开关结构和 p.4 的 initial、sampling、stacking 阶段说明模拟中间结果存储有真实电路与阶段成本。本轮 **没有**将该宏的采样结构当作 1024 槽、240 ns 保持对照的验证。
- 本轮另发现 SACIM-05 资源口径差异：p.2 写 “Each output channel ... 69 ... SAR-ADC”，p.3 写 “each sub-bank ... 96 ... 69 ADCs”。原文给出的层次标签不一致。因此只以同位权合并减少 ADC 的机制为结构依据，96/69 原值保留，不用于当前参考实例的总 ADC 数。这一补充不改变公共范围。
- SDCIM-01 pp.1–2 明确每轮合并 16 项，输入八位后再覆盖八个空间组，得到 64 clocks 的 128×16 VMM。R0 的 32 项不是该宏的原分组；它是 v0.1 已声明、由 SDCIM-03 交叉支持的参考选择。本轮不把 256 拍冒充 D6CIM 实测。

### 结构敏感性的输入、资源与解释

机器入口：`structural_sensitivity`；未取整结果：`data/sensitivity_results.json`。所有情景仍完成 128×128 INT8 的同一服务；ACIM/DCIM 各保持原精度语义。示意物理读时间 ACIM 25 ns、DCIM 10 ns；写入是同一完整直接更新，16 Byte/60 ns。写口与驱动均未变化，且此示意没有共享 ADC 的 verify，所以 τ 保持不变。带 verify 的真实介质须随读资源变化复核写时间。

| 情景 | 操作时间独立复算（ns） | 必要的随动条件 |
|---|---|---|
| A0 | 64×30 + 64×20 + 128×5 + 10 = 3850 | 原 R0 |
| A1 | 32×30 + 32×20 + 128×5 + 10 = 2250 | 每平面 ADC 16→32，总数128→256；求值宽度16→32；批锁存码128→256；数字通道16不变，数字总拍仍128。读侧与时隙不变为条件假设。 |
| A2 | 8×30 + 64×20 + 128×5 + 8×5 + 10 = 2210 | 1024个有效模拟槽及隔离/缓冲；每求值额外5 ns采集/建立；每组顺序处理八批，保守保持至少8×(20+10)=240 ns。负载/噪声/漏电须满足原读出目标。无电路验证，5 ns是人为示意预算。 |
| D0 | 256×10 + 256×5 + 10 = 3850 | 原 R0 |
| D1 | 128×10 + 128×5 + 10 = 1930 | 输出16→32，物理读出和乘法/归约资源同步增加；假设仍满足10 ns读侧和5 ns节拍。 |

A1、A2、D1 的 ρ 与 ridge 倍率分别为 3850/2250≈1.711、3850/2210≈1.742、3850/1930≈1.995。没有比较等面积/等功耗；保持槽数与 ADC 计数均为有效资源数，差分电路可需要更多物理节点。A2 在5 ns采集之外，转换/数字等待已在总时间中，不另加240 ns一次“保持延迟”；240 ns是保持能力要求。读后恢复若破坏已采样结果，应增加隔离或改回重新求值，不能沿用该数值。

小扰动另与结构对照分开：T_A=16/24 ns 只改变转换占用；T_D=4/6 ns 同时改变计算与写控制。生成正文给出带符号的百分比范围。A 路 ridge 对 T_D 小变化约 ±0.04% 稳定，是此特定读写配对的比例接近抵消；并非所有介质的定律。原来两例外围与物理时间同向变化的三点仅作回归，不拿来隔离外围敏感性。

### 检查覆盖与已知边界

检查脚本从通用公式复现 R0 64/256 和原六个算例；枚举 ADC 输出覆盖并检查尾组，检查输入/权重位展开的有用乘积数量；验证完整权重/互补编码/写批数/数据拍、单脉冲循环与完整 program 块、有效页与擦除摊销；检查单位、服务粒度因子、聚合不变性及独立手算的敏感性结果。脚本对保持槽不足、保持时间不足和权重锁存不足拒绝计算，具体模拟误差仍须后续电路依据支持。生成表、敏感性 JSON 与正文入口自动核对，公式的语义另作人工审阅。

以下保留 v0.1 八项证据的原值、详细定位与选择理由；其中“当前配置”现指 R0 实例。原计量文件与原文不改。
...
- 相同位权的 GBL-comb 把 ADC 从 96 个减少为 69 个（p.3 末段，写为每 sub-bank；与 p.2 的每 output-channel 标签存在上述差异）；之后数字 shift-and-add 重构。p.4 Fig.6 有 initial、voltage sampling、voltage stacking 三阶段，原图已核对。
