...
本记录给出八项文献的原值、定位与参考选择依据。页码为本地 PDF 页序，从 1 开始；来源路径与 SHA-256 见共享参数。外围范围属于文献支持的参考设计预算，组合后的服务能力由公式推导。

## 参考配置的操作计数

R0 的 ACIM 每向量执行 8 个输入 bit × 8 个输出组，每组重新求值；16 条数字输出通道每批用两拍合并八个位平面并累加。因此有64次求值、64批转换、128拍重构和8192个有用标量转换结果。DCIM 的 8 个输入 bit × 4 个行组 × 8 个输出组给出256轮计算。

通用计数按操作分工确定：求值组由实际同时产生的输出数确定，ADC 批次由样本与转换资源确定，数字轮数由输出通道与每轮功能确定。保持方案减少求值次数，转换数和同一次逻辑 payload 不因此减少。resident 按完整权重、物理更新批次和编码装入拍数计数，并根据实际步骤加入 verify、恢复与擦除摊销。

## 结构证据的适用范围

- CMOS-02 pp.7–9 提供100 MS/s及ENOB测量条件；其SAR内部单样本采样与跨共享组的长保持资源承担不同功能。
- SACIM-05 p.3 的电容/开关结构和p.4的initial、sampling、stacking阶段说明模拟中间结果存储有实际电路与阶段成本。1024槽、240 ns保持情景是独立设计预算，未经该宏验证。
- SACIM-05 p.2写“Each output channel ... 69 ... SAR-ADC”，p.3写“each sub-bank ... 96 ... 69 ADCs”，资源层次标签不一致。参考设计采用同位权合并减少ADC的机制作为结构依据，96/69保留为文献原值，不用作R0的总ADC数。
- SDCIM-01 pp.1–2每轮合并16项，输入八位后覆盖八个空间组，给出128×16 VMM的64 clocks。R0采用由SDCIM-03交叉支持的32项分组，其256轮计数由R0自身映射推导。

## 结构敏感性的输入、资源与解释

机器入口：`structural_sensitivity`；未取整结果：`data/sensitivity_results.json`。所有情景完成 128×128 INT8 的同一服务；ACIM/DCIM 各采用声明的精度语义。示意物理读时间 ACIM 25 ns、DCIM 10 ns；写入是同一完整直接更新，16 Byte/60 ns。写口与驱动均未变化，且此示意没有共享 ADC 的 verify，所以 τ 保持不变。带 verify 的真实介质须随读资源变化复核写时间。

| 情景 | 操作时间独立复算（ns） | 必要的随动条件 |
|---|---|---|
| A0 | 64×30 + 64×20 + 128×5 + 10 = 3850 | 默认 R0 |
...
小扰动另与结构对照分开：T_A=16/24 ns 只改变转换占用；T_D=4/6 ns 同时改变计算与写控制。生成正文给出带符号的百分比范围。A 路 ridge 对 T_D 小变化约 ±0.04% 稳定，是此特定读写配对的比例接近抵消；并非所有介质的定律。两个算例中，外围与物理时间同时变化的成对情景用于展示范围传播；单参数小扰动用于单独检查外围敏感性。
...
- **本文选择**：以该宏和 SACIM-05 提供本地输入完整服务的量级交叉证据，主参考采用 128 路一位二电平输入，`T_I=2–10 ns`、推荐 5 ns；属于简化输入电路及带缓冲负载的工程预算。未将该文的多位 DAC 当作已测独立一位驱动器，也未提取不存在的分项绝对时间。
...
- **本文选择**：数字节拍 `T_D=2–10 ns`，推荐 5 ns；16 输出通道与 128-bit 普通写口为结构锚点。R0的128×128 逻辑矩阵不是复制八个独立 D6CIM 服务单元，当前行分组也独立选择为 32 项；完整 256 步需按本基线计数。该文的普通写端口未直接提供独立完整写周期。
...
| 纯公共整向量 ACIM 1028/2250/5140 ns，DCIM 516/1290/2580 ns | 由R0调度与资源数推导，不是器件能力标定；正文展示时间向外取整。 |
